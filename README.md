...
錯誤會顯示在主控台；雙擊啟動失敗時也會出現訊息視窗。紀錄檔只留在本機，不會自動上傳，也沒有遙測。既有功能診斷可能擷取本次啟動遊戲的前景視窗用戶區，並記錄本機路徑與程序資訊；分享紀錄前請先檢查內容。未經挑選的診斷截圖及除錯產物不應加入原始碼儲存庫或玩家版補丁；本文件的四張功能示範截圖是明確列入白名單的例外。
